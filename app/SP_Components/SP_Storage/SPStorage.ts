...
import StorageService from "../../interface-models/sp_type_services/StorageService";
...
@Component({
  selector: 'app-sp-storage',
  templateUrl: './SPStorage.html',
  styleUrls: ['./SPStorage.css']
})

export class SPStorage {
  title = "Storage";
  getParam: any;
  account_id: any = "";
  getRFQParamID: any;
  ...
  constructor(private SPQuoationService: SPQuoationService, private routerParam: ActivatedRoute) {
    ...
    console.log(this.routerParam.snapshot.paramMap);
  ...
  createQuotation(SPQuote: StorageService) {
    let payload = {
